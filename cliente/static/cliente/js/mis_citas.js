...
function filterAppointments(filter) {
    ...
    appointmentRows.forEach(row => {
        ...
        if (filter === 'todas') {
            row.style.display = '';
            visibleCount++;
        } else if (filter === 'proximas' && status === 'proxima') {
            row.style.display = '';
            visibleCount++;
...
if (searchInput) {
    searchInput.addEventListener('input', function(e) {
        ...
        appointmentRows.forEach(row => {
            const professionalName = row.querySelector('.professional-info span').textContent.toLowerCase();
            const specialty = row.querySelector('.specialty-badge').textContent.toLowerCase();
            
            if (professionalName.includes(searchTerm) || specialty.includes(searchTerm)) {
                row.style.display = '';
                visibleCount++;
            } else {
                row.style.display = 'none';
...
function showAppointmentDetails(row) {
    ...
    const professionalName = cells[1].querySelector('.professional-info span').textContent;
    const specialty = cells[2].querySelector('.specialty-badge').textContent;
    const typeElement = cells[3].querySelector('.appointment-type');
    const type = typeElement.textContent.trim();
    const address = cells[4].querySelector('.address-text').textContent;
    const date = cells[5].querySelector('.date').textContent;
    const time = cells[5].querySelector('.time').textContent;
    const status = row.dataset.status;
    
    // Populate modal
    document.getElementById('detailProfessional').textContent = professionalName;
    ...
    currentAppointmentData = { professionalName, specialty, type, address, date, time };
...
function cancelAppointment(row, reason) {
    ...
    console.log('Cancelando cita...', {
        professional: row.querySelector('.professional-info span').textContent,
        reason: reason,
...
editButtons.forEach(button => {
    button.addEventListener('click', function() {
        const row = this.closest('tr');
        const professionalName = row.querySelector('.professional-info span').textContent;
        
        showNotification(`Redirigiendo a editar cita con ${professionalName}...`, 'info');
        
        // Aquí iría la lógica para editar
        // window.location.href = `/cliente/citas/editar/${appointmentId}/`;
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Sistema de citas del cliente inicializado correctamente');
